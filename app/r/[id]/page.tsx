...
// v2.0: Implemented a truly bulletproof, recursive parser to handle any valid JSON structure from the AI.
...
import { Logo } from '@/app/components/Logo'; // Note the slightly different path
...
// --- NEW: Bulletproof Data Parsing Function ---
const parseFeedbackItems = (data: any): FeedbackItem[] => {
  const items: FeedbackItem[] = [];
  if (!data || typeof data !== 'object') return items;

  const isFeedbackItem = (item: any): item is Omit<FeedbackItem, 'category'> & { category?: string } => {
    return item && typeof item.sentiment === 'string' && typeof item.summary === 'string' && typeof item.quote === 'string';
  };

  const findItemsRecursively = (obj: any, potentialCategory?: string) => {
...
const ReportPage = () => {
    ...
    const fetchReport = async () => {
      ...
      try {
        ...
        if (data && data.report_data) {
          console.log("Fetched raw data from Supabase:", data.report_data);
  ...
  // Safely get the summary text, whether it's a string or an object
  const getSummaryText = (summary: any): string => {
    if (typeof summary === 'string') return summary;
    if (typeof summary === 'object' && summary !== null) {
      // Try to find a logical key, otherwise join all values
      return summary.summary_text || summary.text || Object.values(summary).join(' ');
    }
                  ...
                  {feedbackItems.length > 0 ? feedbackItems.map((item, index) => (
                    ...
                    >
                      <div className="flex items-center gap-3 mb-3">
                        <span className={`flex items-center gap-2 px-2 py-1 text-xs font-semibold rounded-full ${item.sentiment === 'Positive' ? 'bg-green-500/10 text-green-300' :
                            item.sentiment === 'Negative' ? 'bg-red-500/10 text-red-300' :
                              'bg-gray-500/10 text-gray-300'
                          }`}>
                          <div className={`w-2 h-2 rounded-full ${item.sentiment === 'Positive' ? 'bg-green-400' :
